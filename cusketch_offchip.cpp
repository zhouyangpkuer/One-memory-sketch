...
using namespace std;
CUSketch_off::CUSketch_off(int w, int c, int hw, int hc)
{
...
lint CUSketch_off::Query(const char *str)
{
...
void CUSketch_off::Insert(const char *str)
{
	unsigned long long res = MAX_NUM;
	int temp = 0;
	...
	for(int i = 0; i < hash_counter; i++)
	{
		if((unsigned long long)(sketch[hash_value[i]].counter + offsketch[hash_value[i]].counter * (1 << ON_COUNTER_SIZE)) == res)
		{
...
void CUSketch_off::Delete(const char *str)
{
